...
//
// Copied from Chromium's vertical TabCollectionAnimatingLayoutManager; only
// the class name and include path are changed. See THIRD_PARTY_NOTICES.md.
...
DEFINE_UI_CLASS_PROPERTY_TYPE(
    CmuxRailAnimatingLayoutManager::SourceLayoutInfo*)
...
bool CmuxRailAnimatingLayoutManager::Delegate::IsDragging() const {
  return false;
}

bool CmuxRailAnimatingLayoutManager::Delegate::IsViewDragging(
    const views::View& child_view) const {
  return false;
}

bool CmuxRailAnimatingLayoutManager::Delegate::ShouldSnapToTarget(
    const views::View& child_view) const {
  return false;
}

bool CmuxRailAnimatingLayoutManager::Delegate::
    ShouldAnimateOpacityForAddAndRemove(const views::View& child_view) const {
  return false;
}

void CmuxRailAnimatingLayoutManager::Delegate::OnAnimationEnded() {}

CmuxRailAnimatingLayoutManager::CmuxRailAnimatingLayoutManager(
...
bool CmuxRailAnimatingLayoutManager::OnViewAdded(views::View* host,
                                                      views::View* view) {
...
bool CmuxRailAnimatingLayoutManager::OnViewRemoved(views::View* host,
                                                        views::View* view) {
  ClearViewAnimationMetadataForView(view);
  return LayoutManagerBase::OnViewRemoved(host, view);
}

gfx::Size CmuxRailAnimatingLayoutManager::GetPreferredSize(
...
gfx::Size CmuxRailAnimatingLayoutManager::GetMinimumSize(
    const views::View* host) const {
  return target_layout_manager_->GetMinimumSize(host);
}

int CmuxRailAnimatingLayoutManager::GetPreferredHeightForWidth(
    const views::View* host,
    int width) const {
  return target_layout_manager_->GetPreferredHeightForWidth(host, width);
}

void CmuxRailAnimatingLayoutManager::OnLayoutChanged() {
...
void CmuxRailAnimatingLayoutManager::AnimationProgressed(
...
void CmuxRailAnimatingLayoutManager::AnimationEnded(
    const gfx::Animation* animation) {
  // Do not invalidate the target layout as the animation progresses, only the
  // animating layout manager requires invalidation.
  InvalidateHost(/*mark_layouts_changed=*/false);
...
// static.
void CmuxRailAnimatingLayoutManager::SetSourceLayoutInfo(
...
CmuxRailAnimatingLayoutManager::CalculateProposedLayout(
...
void CmuxRailAnimatingLayoutManager::LayoutImpl() {
...
void CmuxRailAnimatingLayoutManager::OnInstalled(views::View* host) {
  LayoutManagerBase::OnInstalled(host);
  RecalculateTarget();
}

void CmuxRailAnimatingLayoutManager::SetStartingLayout(
...
void CmuxRailAnimatingLayoutManager::SetTargetLayout(
...
void CmuxRailAnimatingLayoutManager::UpdateCurrentLayout() {
...
bool CmuxRailAnimatingLayoutManager::RecalculateTarget() {
  if (!host_view()) {
    return false;
  }
...
void CmuxRailAnimatingLayoutManager::ResetViewsToTargetLayout(
...
void CmuxRailAnimatingLayoutManager::AnimateAndDestroyChildView(
...
void CmuxRailAnimatingLayoutManager::AnimateAndReparentView(
...
views::ProposedLayout CmuxRailAnimatingLayoutManager::InterpolateLayout(
    double value) const {
...
void CmuxRailAnimatingLayoutManager::
    RemoveNonAnimatingPendingDeleteViews() {
...
void CmuxRailAnimatingLayoutManager::ClearViewAnimationMetadata() {
  for (views::View* child_view : host_view()->children()) {
    ClearViewAnimationMetadataForView(child_view);
  }
}

void CmuxRailAnimatingLayoutManager::ClearViewAnimationMetadataForView(
    views::View* view) {
